...
{
    internal sealed class OpenXmlFontReplacer
    {
        ...
        public int ApplyFontReplacements(string filePath, IReadOnlyCollection<string> sourceFonts, string replacementFont)
        {
            ...
            var normalizedFonts = new HashSet<string>(
                sourceFonts.Select(FontNameNormalizer.Normalize).Where(x => !string.IsNullOrWhiteSpace(x)),
            ...
            var results = ProcessQueue(workItems, item => ApplyFontReplacements(item, normalizedFonts, FontNameNormalizer.NormalizeReplacementFont(replacementFont)));
            ...
            AhoCorasickMatcher<string> matcher)
        {
            ...
            {
                foreach (var descriptor in descriptorFactory(document))
                {
                    using (var stream = descriptor.Part.GetStream(FileMode.Open, FileAccess.Read))
                    using (var reader = new StreamReader(stream))
                    {
                        var xml = reader.ReadToEnd();
                        if (string.IsNullOrWhiteSpace(xml) || (matcher != null && !matcher.Matches(xml)))
                        {
                            continue;
                        }
            ...
            string replacementFont)
        {
            ...
            foreach (var fontElement in document.Descendants().Where(IsFontElement).ToList())
            {
                var typeface = FontNameNormalizer.Normalize((string)fontElement.Attribute("typeface"));
        ...
        private static AhoCorasickMatcher<string> BuildMatcher(IEnumerable<string> patterns)
        {
            ...
            if (patterns != null)
            {
                ...
                {
                    var normalized = FontNameNormalizer.Normalize(pattern);
                    if (!string.IsNullOrWhiteSpace(normalized))
                    {
                        matcher.Add(normalized, normalized);
                    }
                }
            }
        ...
        private static IReadOnlyList<PartDescriptor> BuildFontPartDescriptors(PresentationDocument document)
        {
            ...
            var slideOrder = 1;
            var slideIds = document.PresentationPart.Presentation == null || document.PresentationPart.Presentation.SlideIdList == null
            ...
            foreach (var slideId in slideIds)
            {
                ...
                if (slidePart != null)
                {
                    descriptors.Add(new PartDescriptor(slidePart, 1000 + slideOrder));
                    slideOrder++;
                }
            }
        ...
        private sealed class PartDescriptor
        {
            public PartDescriptor(OpenXmlPart part, int sortOrder)
            {
                Part = part;
                SortOrder = sortOrder;
                Uri = part == null || part.Uri == null ? string.Empty : part.Uri.ToString();
            }
